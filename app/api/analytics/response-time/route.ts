...
import type { ResponseTimePoint } from "@/features/analytics/types";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const points = await goApiFetch<ResponseTimePoint[]>(
      `/v1/analytics/response-time?days=${encodeURIComponent(days)}`,
      { accessToken },
    );
