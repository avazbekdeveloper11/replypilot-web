 ...
 * features/dashboard/lib/format.ts's identical helper; this is a separate
 * copy per this project's per-feature-utility convention, not a shared
 * import). Plain utility functions can't call the hook themselves. */
 ...
 * Takes the app's selected locale (see src/i18n/config.ts's intlLocale)
 * rather than `undefined`, so the hour format / separator matches the
